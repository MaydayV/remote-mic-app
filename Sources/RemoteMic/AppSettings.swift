...
final class AppSettings: ObservableObject {
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        if let data = defaults.data(forKey: Keys.buttonRapidPressEnabled),
           let decoded = try? JSONDecoder().decode([String: Bool].self, from: data) {
            buttonRapidPressEnabled = Dictionary(uniqueKeysWithValues: decoded.compactMap { key, value in
                RemoteButton(rawValue: key).map { ($0, value) }
            })
        } else {
    ...
    func allowsRapidPress(for button: RemoteButton) -> Bool { buttonRapidPressEnabled[button] ?? false }
    ...
    func setAllowsRapidPress(_ enabled: Bool, for button: RemoteButton) {
        if enabled { buttonRapidPressEnabled[button] = true }
        else { buttonRapidPressEnabled.removeValue(forKey: button) }
    }
    ...
    func exportedConfigurationData() throws -> Data {
        let configuration = PersonalizedConfiguration(
            ...
            voiceKeyMode: voiceKeyMode,
            buttonRapidPressEnabled: buttonRapidPressEnabled.isEmpty ? nil : Dictionary(
    ...
    func importConfiguration(from data: Data) throws {
        ...
        buttonRapidPressEnabled = Dictionary(uniqueKeysWithValues: (configuration.buttonRapidPressEnabled ?? [:]).compactMap { key, value in
            RemoteButton(rawValue: key).map { ($0, value) }
        })
    ...
    private func saveButtonRapidPressEnabled() {
        let raw = Dictionary(uniqueKeysWithValues: buttonRapidPressEnabled.map { ($0.key.rawValue, $0.value) })
        if let data = try? JSONEncoder().encode(raw) { defaults.set(data, forKey: Keys.buttonRapidPressEnabled) }
    }
