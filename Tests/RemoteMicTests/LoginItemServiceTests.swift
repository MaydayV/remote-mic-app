...
struct LoginItemServiceTests {
    @Test func disabledByDefault() {
        let service = LoginItemService(controller: LoginItemControllerSpy(status: .notRegistered))
    ...
    @Test func approvalStateProvidesSystemSettingsRecovery() {
        let controller = LoginItemControllerSpy(status: .notRegistered)
    ...
    @Test func failedRegistrationReportsFailure() {
        let controller = LoginItemControllerSpy(status: .notRegistered)
...
private final class LoginItemControllerSpy: LoginItemControlling {
    enum Error: Swift.Error { case registerFailed }
    ...
    init(status: LoginItemRegistrationStatus) { self.status = status }
    ...
    func openLoginItemsSettings() { openSettingsCallCount += 1 }
}
