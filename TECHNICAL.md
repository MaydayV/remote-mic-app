...
用户还可以选择向上/向下滚动、系统静音、播放/暂停，或打开 Codex、Claude、cmux、微信、Cursor、Xcode、Slack、企业微信、网易云音乐、Chrome、Safari 和 Zed。滚动事件定位到前台窗口中心，无法取得窗口时回退到当前指针位置。选择器只显示当前已安装的预置应用，但会保留后来被卸载的已有映射；应用启动动作不会重复创建实例。

方向、滚动、返回和音量键支持长按重复；打开应用动作不重复。普通实体按键活动状态会发布到 SwiftUI，用于高亮遥控器示意图和定位映射行。
...
`scripts/publish-release.sh prerelease` 只接受干净、已推送且由同一远端 Tag 指向的源提交，发布 Apple Silicon 与 Intel 两套 ZIP、PKG、DMG、校验文件、按语言区分的更新说明和各自 appcast，并确认 pre-release 未改变 latest release。脚本随后从公开 Release 回下载全部 16 个分发资产并逐字节比较。测试机应使用与架构一致的 Sparkle CLI 单次 `--feed-url <候选版本 appcast URL>` 覆盖完成候选探测或更新，不写入持久化的 `SUFeedURL` 偏好；实际安装候选版本时需要处于已解锁的图形会话。
