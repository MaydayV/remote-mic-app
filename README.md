...
每个普通按键都可以设置单击动作，并可额外设置双击和长按动作。动作支持键盘操作、向上/向下滚动、系统音量、播放控制、打开当前 Mac 已安装的常用应用，以及录入任意自定义键盘快捷键。
...
语音键始终用于语音输入与 Fn 功能，不参与普通按键映射；可在语音键区域选择 Fn/地球键、左 Command、右 Command 或右 Option 长按。
